...
class RougeScoreAggregated(scoring.BaseScorer):
...
def _score_lcs(target_tokens, prediction_tokens):
    ...
    if not target_tokens and not prediction_tokens:
        return scoring.Score(precision=1, recall=1, fmeasure=1)
...
def _summary_level_lcs(ref_sent, can_sent):
    ...
    """
    if not ref_sent and not can_sent:
        return scoring.Score(precision=1, recall=1, fmeasure=1)
...
def _score_ngrams(target_ngrams, prediction_ngrams):
    ...
    return {'matched_ngram_count': intersection_ngrams_count, 'target_ngram_count': target_ngrams_count, 'prediction_ngrams_count': prediction_ngrams_count}
